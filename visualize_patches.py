...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to input image (e.g., kodim04.png)",
    )
    parser.add_argument(
        "-c",
        "--checkpoint",
    ...
    parser.add_argument(
        ...
        help="Crop coordinates: y1 y2 x1 x2",
    )
...
def main():
    ...
    if y2 > orig_np.shape[0] or x2 > orig_np.shape[1] or y1 >= y2 or x1 >= x2:
        print(
            f"Warning: Invalid crop {args.crop} for image size {orig_np.shape}. Plotting full image."
        )
