...
def main():
    ...
    parser.add_argument("--image", type=str, required=True, help="Path to a complex Kodak image (e.g., kodim23.png)")
    ...
    bars = plt.bar(labels, bpps, color=['gray', 'darkred', 'red', 'orange', 'lightcoral', 'mistyrose'])
    ...
    for bar, bpp in zip(bars, bpps):
        yval = bar.get_height()
        plt.text(bar.get_x() + bar.get_width()/2, yval + 0.01, f'{bpp/total_bpp*100:.1f}%', ha='center', va='bottom')
