...
def test_selective_scan(
    ...
    if has_delta_bias:
        delta_bias = (
            0.5 * torch.rand(dim1, device=device, dtype=torch.float32)
        ).requires_grad_()
    else:
        delta_bias = None
    u = torch.randn(
    ...
    delta = (
        0.5 * torch.rand(batch_size, dim1, seqlen, device=device, dtype=itype)
    ).requires_grad_()
    ...
    if dim1 != dim:
        assert dim % dim1 == 0
        delta_ref = (
            delta.unsqueeze(2).repeat(1, 1, dim // dim1, 1).contiguous().flatten(1, 2)
        )
    ...
    if dim == dim1:
        ...
        dgr = delta_ref.grad.view(
            delta_ref.grad.shape[0], -1, dim // dim1, delta_ref.grad.shape[-1]
        ).sum(2)
        print(f"ddelta max diff: {(delta.grad - dgr).abs().max().item()}")
        assert torch.allclose(
            delta.grad, dgr.to(dtype=itype), rtol=rtol * 5, atol=atol * 10
        ), breakpoint()
        if has_delta_bias:
