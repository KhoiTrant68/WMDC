...
def selective_scan_easy(
    ...
    return (
        ...
        else (oys.to(inp_dtype), hprefix.view(B, G * D, N).float())
    )
...
class SelectiveScanEasy(torch.autograd.Function):
    ...
    def backward(ctx, doys: torch.Tensor, *args):
        ...
        for i in chunks[::-1]:
            ...
            dCs[i : i + chunksize] = torch.einsum(
                "lbgd,lbgdn->lbgn", doys[i : i + chunksize], _hs
            )
            dhs = doys[i : i + chunksize].unsqueeze(4) * Cs[
                i : i + chunksize
            ].unsqueeze(
                3
            )  # lbgd,lbgn->lbgdn
            ...
            dAts_hprefix = (
                dhs * _hprefix.unsqueeze_(0) if i > 0 else None
            )  # lbgdn,bgdn->lbgdn
            ...
            dus[i : i + chunksize] = (
                dus[i : i + chunksize] + dduts * dts[i : i + chunksize]
            )
            if DEBUG:
                print(
                    "3",
            ...
            if DEBUG:
                ...
                print(
                    ...
                    (
                        torch.autograd.grad(
                            tmp_b, tmp_a, tmp_c, create_graph=True, allow_unused=True
                        )[0]
                        - rev_comsum(tmp_c)
                    )
                    .abs()
            ...
            if DEBUG:
                drAts_dtBus_div_rAts_v1 = d_dtBus_div_rAts * (
                    dtBus / -(rAts * rAts)
                )  # do not use this!!!
                ...
                print(
                    "4.0", (drAts_dtBus_div_rAts - drAts_dtBus_div_rAts_ref).abs().sum()
                )
                print(
                    "4.0_v1",
            ...
            dAts = (
                drAts_dtBus_div_rAts / scale
                + drAts_hs_minus_prefix / scale
                + (dAts_hprefix if i > 0 else 0)
            )
            ...
            if DEBUG:
                ...
                print(
                    "4.1",
                    (drAts_ref - (drAts_dtBus_div_rAts + drAts_hs_minus_prefix))
                    .abs()
                    .sum(),
                )
                print(
            ...
            if DEBUG:
                _part_ddts_v1 = torch.autograd.grad(
                    ts, tmp_dts, d_ts, create_graph=True, allow_unused=True
                )[0]
        ...
        if DEBUG:
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, dts, doys, create_graph=True, allow_unused=True
                    )[0]
                    - ddts
                )
                .abs()
        ...
        if DEBUG:
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, us, doys, create_graph=True, allow_unused=True
                    )[0]
                    - dus
                )
                .abs()
            ...
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, Bs, doys, create_graph=True, allow_unused=True
                    )[0]
                    - dBs
                )
                .abs()
            ...
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, Cs, doys, create_graph=True, allow_unused=True
                    )[0]
                    - dCs
                )
                .abs()
            ...
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, Ds, doys, create_graph=True, allow_unused=True
                    )[0].view(-1)
                    - dDs
                )
                .abs()
            ...
            print(
                ...
                (
                    torch.autograd.grad(
                        _oys, As, doys, create_graph=True, allow_unused=True
                    )[0]
                    - dAs
                )
                .abs()
...
def selective_scan_easy_fwdbwd(
    ...
    if mode in ["fnDEBUG"]:
        ...
        return (
            (outs[0].to(u.dtype), *outs[1:])
            if return_last_state
            else outs[0].to(u.dtype)
        )
    else:
...
def selective_scan_easyv2(
    ...
    def ss_chunk(us, dts, As, Bs, Cs, h0, mask):
        # BHLD, BHLN, HND, BHDN
        cL = us.shape[2]
        _mask = (mask[:cL, :cL].contiguous() if cL < chunksize else mask).view(
            1, 1, cL, cL, 1
        )

        w_log = As[None, :, None, :, :] * (
            torch.cumsum(dts, dim=2)[..., None, :]
        )  # (B, H, L, Dk, Dv)
        v = us * dts  # (B,H,L,Dv)
        k = Bs  # (B,H,L,Dk)
        q = Cs  # (B,H,L,Dk)
...
def selective_scan_easyv3(
    ...
    def cut_chunk(us, dts, Bs, Cs, chunksize=chunksize):
        ...
        _pad = lambda x: torch.nn.functional.pad(
            x.view(B * H, L, -1), (0, 0, 0, to_pad, 0, 0)
        ).view(B * H, NT, chunksize, x.shape[-1])
        us, dts, Bs, Cs = _pad(us), _pad(dts), _pad(Bs), _pad(Cs)
        return us, dts, Bs, Cs

    def ss_chunk_h1y1(
        qs, ks, vs, ws=None, As=None, ts=None, dts=None, mask=None, scale=1
    ):
    ...
    ht1, y1, ws, cws, q_mul_w = ss_chunk_h1y1(
        Cs, Bs, _us * dts, None, As, None, dts, mask=_mask_tril
    )
...
class SelectiveScanMatrix(torch.autograd.Function):
    ...
    def forward(
        ...
        if chunksize < 1:
            chunksize = Bs.shape[-1]
        mask = torch.tril(
            torch.ones((chunksize, chunksize), device=us.device), diagonal=0
        )

        def ss_chunk(us, dts, As, Bs, Cs, h0, mask):
            # BHLD, BHLN, HND, BHDN
            cL = us.shape[2]
            _mask = (mask[:cL, :cL].contiguous() if cL < mask.shape[0] else mask).view(
                1, 1, cL, cL, 1
            )
        ...
        return (
            oys.to(inp_dtype) if not return_last_state else (oys.to(inp_dtype), hprefix)
        )
    ...
    def backward(ctx, doys: torch.Tensor, *args):
        ...
        for i in chunks[::-1]:
            ...
            if True:
                ...
                d_v_nexth1 = torch.einsum(
                    "bhkv,bhlkv->bhlv", d_nexth1tmp_nexth1, k_div_w
                )
...
def selective_scan_easyv2_fwdbwd(
    ...
    outs = SelectiveScanMatrix.apply(
        u, delta, A, B, C, D, delta_bias, delta_softplus, return_last_state, chunksize
    )
    return (
        (outs[0].to(u.dtype), *outs[1:]) if return_last_state else outs[0].to(u.dtype)
    )
