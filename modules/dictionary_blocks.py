...
class UnifiedDictionaryAttention(nn.Module):
    ...
    def forward(self, x, k, v, spatial_epsilon, calc_disp=False):
        ...
        if self.routing_mode == "softmax":
            ...
            eps = spatial_epsilon.mean(dim=(2, 3)).view(B, 1).clamp(min=1e-4)  # (B, 1)
            ...
            eps = spatial_epsilon.mean(dim=(2, 3)).view(B, 1).clamp(min=1e-4)
            ...
            P = P / (P.sum(dim=-1, keepdim=True) + 1e-6)  # Normalize rows to 1
